...
ggplot() + theme_bw() + 
  geom_point(data = minimaIco, aes(x = minimaIco$N_Ag+0.25, 
                                   y = minimaIco$Gap,colour=minimaIco$Type),
                                   shape = 15,
             #             colour = "#56B4E9", 
             size=3.5,lwd=3) + 
  geom_point(data = AuAgIco, aes(x = AuAgIco$N_Ag+0.25, y = abs(AuAgIco$Gap),colour= AuAgIco$Type),
             shape=0,size=3.5, 
             #             colour="#56B4E9",
             lwd=3,width = I(3)) +
  ...
  geom_point(data = minimaDeca, aes(x = minimaDeca$N_Ag-0.25, 
             ...
             #colour = "#D55E00", 
             size=5,lwd=3) + 
  geom_point(data = AuAgDeca, aes(x = AuAgDeca$N_Ag-0.25, y = abs(AuAgDeca$Gap),color=AuAgDeca$Type),
             size=3.5, 
             #colour="#D55E00",
             shape=5,width = I(3),lwd=3 ) +
  geom_point(data = minimaCubo, aes(x = minimaCubo$N_Ag-0.0833, 
  y = minimaCubo$Gap,color=minimaCubo$Type),
                                    shape = 17,
#             colour = "#009E73",
             size=3.5,lwd=3) + 
  scale_linetype_discrete('Minima') + 
  geom_point(data = AuAgCubo, aes(x = AuAgCubo$N_Ag-0.0833, y = abs(AuAgCubo$Gap),colour=AuAgCubo$Type),
  size=3.5 , #) + #, 
#             colour="#009E73",
             shape=2,width = I(3) ) +
#  ) + # % position = "jitter") + 
...
#  annotate("text", x = 10, y = 0.7, label = "Deca",colour="#D55E00") +
  ...
  geom_point(data = minimaBBP, aes(x = minimaBBP$N_Ag+0.0833, 
                                   y = minimaBBP$Gap,color=minimaBBP$Type),
             shape = 16,width = I(0.6),
             size=3.5,show_guide = FALSE) + 
  geom_point(data = AuAgBBP, aes(x = AuAgBBP$N_Ag+0.0833, y = abs(AuAgBBP$Gap), color=AuAgBBP$Type ),
             size=3.5, width = I(3),
             #             colour="black",
             shape=21) +
  geom_line(data = minimaIco, aes(x = minimaIco$N_Ag+0.25, y = minimaIco$Gap,colour=minimaIco$Type),
            linetype = "solid",
            # "Minima E Ico"),
            #colour = "#56B4E9",
            size = 1) + 
  geom_line(data = minimaDeca, aes(x = minimaDeca$N_Ag-0.25, y = minimaDeca$Gap,
                                   color=minimaDeca$Type),linetype = "solid",
            # "Minima E Deca"),
            #colour = "#D55E00",
            size = 1) + 
  geom_line(data = minimaCubo, aes(x = minimaCubo$N_Ag-0.0833, y = minimaCubo$Gap,
                                   color=minimaCubo$Type),
            linetype = "solid", #"Minima E Cubo"),
            #colour = "#009E73", 
            size = 1) + 
  geom_line(data = minimaBBP, aes(x = minimaBBP$N_Ag+0.0833, 
                                  y = minimaBBP$Gap,color=minimaBBP$Type),linetype = "solid",
            # "Minima E BBP"),
            #colour = "black",
            size = 1) + 
  #annotate("text", x = 4, y = 0.7, label = "BBP",colour="black")+
  xlab("Number of Ag Atoms") + ylab("Gap (eV)")+
  #labs(colour="Gap",size="Spin") +
  #labs(title = "Gap vs Silver Atoms", colour="Spin" ) + 
...
#  scale_x_continuous(expand=c(0,0))+ scale_y_continuous(expand=c(0,0)) +
  ...
  scale_y_continuous(limits=c(0,0.45),expand=c(0.0,0.01),
  breaks=seq(0, 0.45, 0.1)) + theme(legend.title = element_text(size=15,
  face="bold" ), legend.text = element_text( size = 20, face = "bold"),
...
#  "#009E73"  
ggsave(file = "GapComparado.pdf", width = 10, height = 7 )
#dev.off()
#png(file = "EnergyExcessDeca.png", bg="transparent")
ggsave(file = "GapComparado.png", bg="transparent",width = 10, height = 7)
