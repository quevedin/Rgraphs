...
write.table(minSYMBBPJoin, file = "foo.csv", sep = ",", 
            col.names = NA, 
            append=TRUE, qmethod = "double")
write.table(minSYMCuboJoin, file = "foo.csv", sep = ",", 
            col.names = NA, 
            append=TRUE, qmethod = "double")
write.table(minSYMDecaJoin, file = "foo.csv", sep = ",", 
            col.names = NA, 
            append=TRUE, qmethod = "double")
write.table(minSYMIcoJoin, file = "foo.csv", sep = ",", 
            col.names = NA, 
            append=TRUE, qmethod = "double")
...
#    title = "Energy Excess vs Silver Atoms (Deca)" , #colour="Gap",size="Spin",size="Spin",
  ...
  guides(colour = guide_legend(override.aes = list(shape= c(22,22,22,22,22,23,21),
          size=c(4,4,4,4,4,5,2.5), width = c(3,3,3,3,3,3,3),lwd=c(3,3,3,3,3,3,3) )))+
...
#        panel.border = element_rect(colour = "black",size = 1) )
  ...
  #                               ) #+
  #labs(linetype="")
ggsave(file = "EnergyExcessDecaSym.pdf", width = 10, height = 7 )
dev.off()
##################################################################################

###############################################################################
ggplot() + theme_bw() + 
  geom_point(data = subset(AuAgDeca,(Schoenflies.symbol.T=="NO")),
             # AuAgDeca
             aes(x = N_Ag, y = EEFinal, #colour=Gap, # size = abs(Spin),
                 colour=Schoenflies.symbol.T),width = I(3),lwd=3,alpha = I(0.75),size=2.5,shape=21) + 
  geom_point(data = subset(AuAgDeca,(Schoenflies.symbol.T=="D5h")),
             # AuAgDeca
             aes(x = N_Ag, y = EEFinal, #colour=Gap, # size = abs(Spin),
                 colour=Schoenflies.symbol.T),alpha = I(1.0),size=5,shape=23,
             width = I(3),lwd=3) +
  geom_point(data = subset(AuAgDeca,(Schoenflies.symbol.T!="NO" & Schoenflies.symbol.T!="D5h")),
             # AuAgDeca
             aes(x = N_Ag, y = EEFinal, #colour=Gap, # size = abs(Spin),
  ...
  guides(colour = guide_legend(override.aes = list(
    shape = c( rep( 22,length( levels(minimaDeca$Schoenflies.symbol.T) ) - 5 ), 23, 22, 21, 22, 22 ),
    size  = c(rep(4,length(levels(minimaDeca$Schoenflies.symbol.T))-5 ),5,4,2.5,4,4 ),
    width = c(rep(3,length(levels(minimaDeca$Schoenflies.symbol.T)) ) ),
    lwd   = c(rep(3,length(levels(minimaDeca$Schoenflies.symbol.T)) ) ) ) ) )+
  scale_size_manual(values = c(3, 5), guide = FALSE) +
  theme(legend.key = element_blank(),
        legend.position = c(.945, .38),
        legend.background=element_blank(),
        panel.grid.major = element_line(size = 0.25, colour="#858585",linetype = "longdash"),
        panel.grid.minor.y = element_line(size = 0.25, colour="#858585",linetype = "dotted"),
        #panel.grid.minor.x = element_blank(),
  ...
  #                               ) #+
  #labs(linetype="")
ggsave(file = "EnergyExcessDecaSym-T.pdf", width = 10, height = 7 )
##################################################################################
...
ggplot() + theme_bw() + 
  ...
  #                               ) #+
  #labs(linetype="")
ggsave(file = "EnergyExcessIcoSYM.pdf", width = 10, height = 7 )
###############################################################
...
ggplot() + theme_bw() + 
  ...
  #                               ) #+
  #labs(linetype="")
ggsave(file = "EnergyExcessIcoSYM-T.pdf", width = 10, height = 7 )
###############################################################
...
ggplot() + theme_bw() + 
  ...
  #                               ) #+
  #labs(linetype="")
ggsave(file = "EnergyExcessCuboSYM.pdf", width = 10, height = 7 )
#############################################
...
ggplot() + theme_bw() + 
  ...
  #                               ) #+
  #labs(linetype="")
ggsave(file = "EnergyExcessCuboSYM-T.pdf", width = 10, height = 7 )
#############################################
...
ggplot() + theme_bw() + 
  ...
  #                               ) #+
  #labs(linetype="")
ggsave(file = "EnergyExcessBBPSYM.pdf", width = 10, height = 7 )
...
ggplot() + theme_bw() + 
  ...
  #                               ) #+
  #labs(linetype="")
ggsave(file = "EnergyExcessBBPSYM-T.pdf", width = 10, height = 7 )
